...
export async function createOpenAICompatibleAdapter(
  input: { apiKey: string | null; model: string; baseUrl: string; timeoutMs: number; headers: Record<string, string>; toolsEnabled: boolean },
  ...
  return new OpenAIAdapter(client, { kind: 'openai-compatible', model: input.model, baseUrl: input.baseUrl, timeoutMs: input.timeoutMs, toolsEnabled: input.toolsEnabled }, sdk)
}
